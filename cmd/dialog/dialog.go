...
import (
	...
	internalhttp "github.com/filatkinen/socialnet/internal/dialog/http"
	"log"
	"os"
	"os/signal"
	"syscall"
...
func main() {
	flag.StringVar(&configFile, "config", "configs/dialog.yaml", "Path to configuration file")
	flag.Parse()

	conf, err := dialog.NewConfig(configFile)
	if err != nil {
