...
import (
	...
	dialogappcas "github.com/filatkinen/socialnet/internal/dialogcas/app"
...
type DialogService struct {
	srv        *http.Server
	log        *log.Logger
	config     dialogcas.Config
	httplog    *httplog
	app        *dialogappcas.DialogApp
	reqCounter *RID
	promData   *promData
...
func NewServer(config dialogcas.Config, log *log.Logger) (*DialogService, error) {
	httpsrv := &http.Server{
	...
	app, err := dialogappcas.New(log, config)
	if err != nil {
		return nil, err
	}
